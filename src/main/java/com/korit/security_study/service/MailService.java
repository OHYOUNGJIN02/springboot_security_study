...
public class MailService {
    ...
    public ApiRespDto<?> sendMail(SendMailReqDto sendMailReqDto, Principal principal) {
        ...
        message.setSubject("[이메일을 인증해주세요]");
        ...
        return new ApiRespDto<>("success", "인증 메일이 전송 되었습니다.", null);
    ...
    public Map<String, Object> verify(String token) {
        ...
        try{
            ...
            if (!"VerifyToken".equals(subject)){
                return resultMap = Map.of("status", "failed",
                        "message", "잘못된 접근 입니다.");
            }
            ...
            if (tempUserRole.isEmpty()) {
                return Map.of("status", "failed", "message", "이미 인증이 완료된 사용자입니다.");
            }
            ...
            return resultMap = Map.of("status", "failed", "message", "만료된 인증 요청입니다. /n 인증 메일을 다시 요청해주세요. ");
        } catch (JwtException e){
            resultMap = Map.of("status", "failed", "message", "문제가 발생했습니다. /n 인증 메일을 다시 요청해주세요. ");
    }
        return Map.of("status", "success", "message", "성공");
}}
